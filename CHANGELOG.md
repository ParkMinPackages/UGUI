...
### Breaking Changes
- Removed the default cancellation token from `UIActivator` asynchronous transition methods so callers must provide one explicitly.

## [12.3.0] - 2026-08-25

### Added
- Added configurable Update or LateUpdate scheduling and time-scale independence to `UIActivator` animations for DOTween and LitMotion.

### Changed
- Replaced the custom `UIActivator` editor with Odin preview and animation-creation utilities.
- Reorganized `UIActivator` settings and clarified the inactive layout exclusion option while preserving serialized values.
- Disabled initial-state settings while Play Mode is running.

## [12.2.3] - 2026-08-17

### Changed
- Assigned the shared UI animation script icon to the LitMotion Move From active and deactivate components.

## [12.2.2] - 2026-08-16

### Added
- Added a LitMotion `Add Move From` Inspector utility that adds matching active and deactivate animations to `UIActivator`.
...
### Added
- Added the optional `throwIfTransitioning` parameter to `ActiveAsync` and `DeactivateAsync`.

### Changed
- Return immediately instead of throwing when a transition is already running and `throwIfTransitioning` is disabled.
- Moved Git and NuGet dependency metadata from `package.json` to `parkmin-dependencies.json` while retaining Unity Registry dependencies.
